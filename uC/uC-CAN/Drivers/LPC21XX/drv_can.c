...
*
* MODIFIED: directly specifies RJW and SAM (Resynchronization Jump Width and Samples)
* Adds DLC error checking and disables Self Reception (C1CMR, C2CMR).  (See <<)
*********************************************************************************************************
*/
 ...
 *
 * Constants calculated for 10Mhz clock
 */
...
CPU_INT16S LPC21XXCANInit(CPU_INT32U arg)
{
    ...
    AcceptanceTblPtr = LPC21XX_CAN_ACCEPTANCE_FILTER; /* initialize acceptance table pointer      */
                                                      ...
                                                      /*------------------------------------------*/
    if (arg == LPC21XX_CAN_BUS_0) 
    {                                                 /* Check which can bus to initialise        */
        ...
        baud.ResynchJumpWith = LPC21XX_CAN_DEF_RJW;   /* << */
        baud.SAM             = LPC21XX_CAN_DEF_SAM;   /* << set for 3 sample points                  */
                                                      /*------------------------------------------*/
    ...
    switch (paraId) {
        case LPC21XX_CAN_BUS_0:
            if ((LPC21XX_CAN_C1GSR & 1) == 1) {       /* data is available                        */
				...
				if ( frm->DLC > 8 )                   /* << check for illegal DLC                    */
                {
                  result = LPC21XX_CAN_DLC_ERR;
                  break;
                }
...
CPU_INT16S LPC21XXCANWrite (CPU_INT16S paraId, CPU_INT08U *buffer, CPU_INT16U size)
{
    ...
    switch (paraId) {
        case LPC21XX_CAN_BUS_0:
            ...
            LPC21XX_CAN_C1CMR = 0x21UL;               /* << changed from 0x30.  */
													  /* See Errata ES_LPC2119/29 CAN.7 */
													  /* This device may not correctly receive a message */
													  /* if it was also trying to transmit but lost arbitration */
        ...
        case LPC21XX_CAN_BUS_1:
            ...
            LPC21XX_CAN_C2CMR = 0x21L;               /* << changed from 0x30 */	 													  /* See Errata ES_LPC2119/29 CAN.7 */
													 /* This device may not correctly receive a message */
													 /* if it was also trying to transmit but lost arbitration */
            result = size;
            break;
