...
/*                                    GLOBAL OS VARIABLES                                         */
/*------------------------------------------------------------------------------------------------*/
extern OS_SEM   CANOS_TxSem[CANBUS_N];
extern OS_SEM   CANOS_RxSem[CANBUS_N];
...
CPU_INT16S CANOS_Init(void)
{
    ...
    CPU_INT16U  err;                                  /* Local: Errorcode of OS function          */
                                                      /*------------------------------------------*/
    for (i = 0; i < CANBUS_N; i++) {                  /* loop through all busses                  */
...
void CANOS_ResetRx(CPU_INT16S busId)
{
    ...
    OSSemSet(&CANOS_RxSem[busId], 0, &err);           /* reset semaphore counter value            */
...
void CANOS_ResetTx(CPU_INT16S busId)
{
    ...
    OSSemSet(&CANOS_TxSem[busId],                     /* reset semaphore counter value            */
             CANBUS_TX_QSIZE - 1,
             &err);
    if (err != CANOS_NO_ERR) {                        /* see, if no error is detected             */
        can_errnum = CAN_ERR_OSSEM;                   /* set error indication                     */
...
CPU_INT08U CANOS_PendRxFrame(CPU_INT16U timeout, CPU_INT16S busId)
{
    ...
    OSSemPend (&CANOS_RxSem[busId],                   /* Wait for a received frame with timeout   */
               timeout,
...
void CANOS_PostRxFrame(CPU_INT16S busId)
{
    ...
    OSSemPost (&CANOS_RxSem[busId],                   /* signal a received frame                  */
               OS_OPT_POST_ALL,
               &err);
...
CPU_INT08U CANOS_PendTxFrame(CPU_INT16U timeout, CPU_INT16S busId)
{
...
#endif
    OSSemPend (&CANOS_TxSem[busId],                   /* Get semaphore for memory partition       */
               timeout,
               OS_OPT_PEND_BLOCKING,
               (CPU_TS *)0,
               &err);
    ...
    if (err == CANOS_NO_ERR) {                        /* see, if no error is detected             */
        result     = 1;                               /* yes: space for frame in transmit buffer  */
    } else {                                          /* otherwise: an error is detected          */
        can_errnum = CAN_ERR_OSSEMPEND;               /* set error indication                     */
        result     = 0;                               /* no free space in transmit buffer         */
    }
    return (result);                                  /* return function result                   */
}
...
void CANOS_PostTxFrame(CPU_INT16S busId)
{
    ...
    OSSemPost (&CANOS_TxSem[busId],                   /* release transmit buffer ressource        */
               OS_OPT_POST_ALL,
               &err);
...
*
*            This function shall receive a time from the underliing OS. The time value will have
*            the resultion and format of the OS.
*
* \note      This function is a wrapper around the wanted operating system. The following
*            implementation is an implementation for µC/OS-III.
*
