...
static CPU_INT08U CanBus_IsInitialized = 0;           /* marker for: CanBus module is initialized */
...
CPU_INT16S CanBusInit(CPU_INT32U arg)
{
    ...
    if (CanBus_IsInitialized == 0) 
    {
        err = CANOS_Init();                           /* Initialize OS specific services          */
        if (err != CAN_ERR_NONE) 
        {                    /* See, if services are initialized         */
            can_errnum = err;
            return err;                               /* set function result to 'error'           */
        } 
        else 
        {                                      /* otherwise, build free list:              */
            for (i=0; i<CANBUS_N; i++) 
            {              /* loop through all CAN busses in table     */
                bus->RxTimeout = 0;                   /* set timeout to 0 (Rx in blocking mode)   */
                bus->TxTimeout = 0;                   /* set timeout to 0 (Tx in blocking mode)   */
                bus->Dev       = 0;                   /* clear device identifier                  */
                bus->Cfg       = NULL_PTR;            /* clear pointer to bus configuration       */
                bus->BufRxRd   = 0;                   /* set receive read location to buffer      */
                bus->BufRxWr   = 0;                   /* set receive write location to buffer     */
                bus->BufTxRd   = 0;                   /* set transmit read location to buffer     */
                bus->BufTxWr   = 0;                   /* set transmit write location to buffer    */
#if CANBUS_STAT_EN > 0
                bus->RxLost    = 0;                   /* clear counter for lost RX frames         */
                bus->TxLost    = 0;                   /* clear counter for lost TX frames         */
                bus->RxOk      = 0;                   /* clear counter for received RX frames     */
                bus->TxOk      = 0;                   /* clear counter for transmitted TX frames  */
#endif  /* CANBUS_STAT_EN > 0 */

                bus++;                                /* switch bus to next bus in table          */
            }
            
            CanBus_IsInitialized = 1;                 /* mark bus initialized                     */
            
                                                      /* clear OD reporting errors                */
    ...
    {                                          /* otherwise: bus is initialized            */
        result = CAN_ERR_BUSINIT;                     /* indicate: CanBus is already initialized  */
    }                                                 /*------------------------------------------*/
...
CPU_INT16S CanBusIoCtl(CPU_INT16S busId, CPU_INT16U func, void *argp)
{
    ...
    if ((busId < 0) || (busId >= CANBUS_N)) {         /* is busId out of range?                   */
...
CPU_INT16S CanBusRead(CPU_INT16S busId, void *buffer, CPU_INT16U size)
{
    ...
    if (frx != 0) {                                   /* See if frame received within timeout     */
        ...
        if (bus->BufRxRd >= CANBUS_RX_QSIZE) {        /* see, if end of buffer reached            */
            bus->BufRxRd = 0;                         /* yes: reset to start of buffer            */
        }
        CPU_CRITICAL_EXIT();                          /* disable all interrupts                   */
        CanBusCpy(buffer,(void *)frm, sizeof(CANFRM)); /* copy can frame to buffer                */
        result = sizeof(CANFRM);                      /* set received byte counter                */
    }
...
CPU_INT16S CanBusWrite(CPU_INT16S busId, void *buffer, CPU_INT16U size)
{
    ...
    result = cfg->IoCtl(bus->Dev,                     /* get CAN bus device tx buffer status      */
                        ...
                        (void*)&txstatus);
    CANSetErrRegister(result);
    if (txstatus == CAN_TRUE) {                       /* see if canbus tx buffer is ready/idle    */
        ...
        if (ftx != 0u) {                              /* see if we have space in transmit buffer  */
            ...
            CanBusCpy((void *)frm,                    /* copy buffer to can frame memory          */
                       buffer,
                       sizeof(CANFRM));
            result = CAN_ERR_NONE;                    /* indicate successfull transmission        */
        } else {                                      /* otherwise: buffer is full                */
            result = CAN_ERR_UNKNOWN;                 /* indicate error during transmission       */
        }
        CPU_CRITICAL_EXIT();                          /* enable all interrupts                    */  
    }
#else //JML added based on uC/CAN 2.42 release notes, and email from David Coon (CANBUS_TX_HANDLER_EN is enabled, so this has no effect)
    else{
        CPU_CRITICAL_EXIT();                          /* enable all interrupts                    */
        CANOS_PostTxFrame(busId);                      /* release transmit buffer reservation */
    }
...
CPU_INT16S CanBusEnable(CANBUS_PARA *cfg)
{
    ...
    if (bus->Cfg == NULL_PTR) {                       /* this should be disabled                  */
        ...
        if (bus->Dev >= 0) {                          /* See, if bus can be opened                */
            ...
            if (cfg->Baudrate != 0) {                 /* see, if baudrate is configured           */
...
void CanBusTxHandler(CPU_INT16S busId)
{
    ...
    if (bus->BufTxRd != bus->BufTxWr) {               /* see, if buffer contains pending frames   */
        ...
        if (bus->BufTxRd >= CANBUS_TX_QSIZE) {        /* see, if buffer end is reached            */
            bus->BufTxRd = 0;                         /* wrap around to start of ringbuffer       */
        }
...
void CanBusRxHandler(CPU_INT16S busId)
{
    ...
    CANFRM       rxfrm;                               /* Local: received CAN frame                */
#if CPU_CFG_CRITICAL_METHOD == CPU_CRITICAL_METHOD_STATUS_LOCAL
    CPU_SR cpu_sr;                                    /* LocaL: Storage for CPU status register   */
#endif
                                                      /*------------------------------------------*/
    ...
    rxnext = bus->BufRxWr + 1;                        /* calc write location to next receiption   */
    if (rxnext >= CANBUS_RX_QSIZE) 
    {                                                 /* see, if en of buffer is reached          */
        rxnext = 0;                                   /* wrap around to start of buffer           */
    }
    if (rxnext != bus->BufRxRd) 
    {                                                 /* see, if no buffer overrun occurs         */
        bus->BufRxWr = rxnext;                        /* set next location in buffer              */

        err = cfg->Read(bus->Dev, (void *)&rxfrm,     /* read can frame from can bus interface    */
                        sizeof(CANFRM));
        CANSetErrRegister(err);
        CPU_CRITICAL_EXIT();                          /* allow all interrupts                     */

#if CANBUS_HOOK_RX_EN == 1
        consumed = CanBusRxHook(busId, (void *)&rxfrm);
#endif /* CANBUS_HOOK_RX_EN == 1 */

        if (consumed == 0) 
        {                          /* see, if CAN frame is not consumed        */
            CanBusCpy((void *)frm,                    /* copy can frame memory to buffer          */
                      (void *)&rxfrm,
                      sizeof(CANFRM));
...
#if CANBUS_STAT_EN > 0
    if((frm == NULL_PTR) || (err != sizeof(CANFRM))) 
