...
void CanFrmSet(CANFRM *frm, CPU_INT32U value, CPU_INT08U width, CPU_INT08U pos)
{
    ...
    if ((width + (pos & (CPU_INT08U)(~CANFRM_CODING_MSK))) > 64) {
        can_errnum = CAN_ERR_FRMWIDTH;
        return;
    }
  #endif
    ...
    if (width == 1) {                                 /* see, if width indicates 'byte'           */
        if (pos <= 7) {                               /* yes: see, if position is in range        */
            data  = frm->Data + pos;                  /* calculate byte pointer to given position */
            *data = (CPU_INT08U)(value);              /* copy byte to CAN frame payload           */
        }
    } else if (width == 2) {                          /* otherwise see, if width indicates 'word' */
        if (pos <= 6) {                               /* yes: see, if position is in range        */
            data = frm->Data + pos;                   /* calculate byte pointer to given position */
                                                      /*------------------------------------------*/
    ...
    } else if (width == 4) {                          /* otherwise see, if width indicates 'long' */
        if (pos <= 4) {                               /* yes: see, if position is in range        */
            data = frm->Data + pos;                   /* calculate byte pointer to given position */
                                                      /*------------------------------------------*/
            if (coding == CANFRM_BIG_ENDIAN) {        /* see, if big endian in payload           */
    ...
    if ((width <= 32) &&                              /* conditions to set bits in frame          */
       ((width + pos) <= 64)) {

        valuemask = 1;                                /* set mask for first bit                   */
        ...
        if (((pos % 8) == 0) &&                       /* check if we can copy bytes               */
            ((width % 8) == 0)) {
            if (coding == CANFRM_BIG_ENDIAN) {        /* if so check endianess                    */
                while (width > 0) {
                    *data++ = (CPU_INT08U)(value >> (width - 8));
                    width -= 8;
                }
            } else {
                while (width > 0) {
            ...
            if (coding == CANFRM_BIG_ENDIAN) {        /* see, if big endian in payload            */
                ...
                for (i=0;i<width;i++) {               /* copy single bits                         */
                    ...
                    datamask = 0x80 >> bits;          /* prepare mask for data                    */
                    if ((value & valuemask) != 0) {   /* set bit                                  */
                        *data = *data | datamask;
                    } else {                          /* clear  bit                               */
                        *data = *data & ~datamask;
                    }
                    valuemask = valuemask >> 1;       /* prepare mask for next bit in value       */
                    pos++;
                }
            } else {                                  /*------------------------------------------*/
                                                      /* little endian                            */
    ...
    if (width == 1) {                                 /* see, if width indicates 'byte'           */
        ...
        if (pos <= 6) {                               /* yes: see, if position is in range        */
            data = frm->Data + pos;                   /* calculate byte pointer to given position */
                                                      /*------------------------------------------*/
            if (coding == CANFRM_BIG_ENDIAN) {        /* see, if big endian in payload            */
                result  = (CPU_INT32U)(*data) << 8;   /* yes: get MSB                             */
                data++;
                result |= (CPU_INT32U)(*data);        /*      get LSB                             */
            } else {                                  /* otherwise: little endian in payload      */
                result  = (CPU_INT32U)(*data);        /* yes: get LSB                             */
                data++;
                result |= (CPU_INT32U)(*data) << 8;   /*      get MSB                             */
            }
        }
    } else if (width == 4) {                          /* otherwise see, if width indicates 'long' */
        if (pos <= 4) {                               /* yes: see, if position is in range        */
