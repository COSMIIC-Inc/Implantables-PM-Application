...
*
*               This function is a Clock initialization function & MAY be called by application/
...
void  Clk_Init (CLK_ERR  *p_err)
{
    ...
    //Clk_ExtTS_Init();  //- set in RTC time
...
*
*               This function is a Clock to operating system (OS) function & SHOULD be called only
...
* Caller(s)   : Your Product's Application.
*
*               This function is a Clock periodic signal function & MAY be called by application/
*               BSP function(s).
*
...
* Argument(s) : none.
*
* Return(s)   : Get external time stamp (in seconds, UTC+00).
*
...
{
  CLK_TS_SEC secsOfCurrentYear = SEC + 60 * MIN + 3600 * HOUR + DOY * (3600 * HOUR);
  return (YEAR - 1 - 1970) * (3600 * 24 * 365 ) + secsOfCurrentYear;
}
/*$PAGE*/
/*
*********************************************************************************************************
...
CPU_BOOLEAN  Clk_GetDateTime (CLK_DATE_TIME  *p_date_time)
{
  ...
  //  {
      ...
      currentTime.TZ_sec = (CLK_TZ_SEC)18000;
      
      //Clk_SetDateTime (&currentTime);
  
      p_date_time = &currentTime;
//    }
//    else
//    {
...
*               day         Day   value [   1 to   31].
*
* Return(s)   : Day of week [1 to 7], if NO error(s).
*
...
* Argument(s) : p_date_time     Pointer to variable that will receive the date/time structure.
*
*               yr              Year    value [1900 to 2035] (see Note #1).
*
*               month           Month   value [   1 to   12].
*
*               day             Day     value [   1 to   31].
...
* Argument(s) : p_date_time     Pointer to variable that will receive the date/time structure.
*
*               yr              Year    value [1970 to 2105] (see Note #1).
*
*               month           Month   value [   1 to   12].
*
*               day             Day     value [   1 to   31].
...
*
* Argument(s) : day_of_wk   Day of week value [1 to 7].
*
                                        ...
                                        CLK_DAY    day)
{
    ...
    for (yr_ix = CLK_NTP_EPOCH_YR_START; yr_ix < yr; yr_ix++) { /* See Note #1.                                         */
        leap_yr     =  Clk_IsLeapYr(yr_ix);
        leap_yr_ix  = (leap_yr == DEF_YES) ? 1u : 0u;
        days       +=  Clk_DaysInYr[leap_yr_ix];
    }

    days      += CLK_NTP_EPOCH_DAY_OF_WK;                       /* See Note #1.                                         */
                                                ...
                                                CLK_YR          yr_end)
{
    ...
    if (valid != DEF_YES) {
        return  (DEF_FAIL);
                                                                ...
                                                                /* ---------------- VALIDATE DAY OF YR ---------------- */
    valid = Clk_IsDayOfYrValid(yr, p_date_time->DayOfWk);
    if (valid != DEF_YES) {
        return  (DEF_FAIL);
    }
...
/* printf defined here to get rid of compiler warnings */
//void printf(CPU_CHAR *format, ...)
//{
//  
//  
