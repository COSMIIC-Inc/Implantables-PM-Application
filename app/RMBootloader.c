 ...
 // CPU_INT16U bootRetries = MAX_RETRIES_BOOTLOADER;
   ...
   case RM_ERASE_MEM:
    {
     // bulk erase selected memory location   
      ...
      // can erase lower and upper page, so no page select necessary
      m.cob_id  = 0x146; 
  ...
  case RM_BLANK_CHECK:
    {
      ...
      m.len = 3;
      canSend(0 ,&m);
      //wait for response but don't do anything with it
      pendGatewaySem(CAN_BL_SHORTTIMEOUT_TICKS);
      
      flashReadFlag = 1; 
      copySize = 2;
      transferLength = 0; // reset since it's additive
      ...
      // wait until memory clears in target
      ...
      // blank check the second page
      m.cob_id  = 0x146; 
      m.data[0] = 0x02; //set page only
      m.data[1] = 0x00; //ignored 
      m.data[2] = 0x01; //page = upper
      m.len = 3;
      canSend(0 ,&m);
      //wait for response but don't do anything with it
  ...
  for ( index = 0; index < MAX_TRANSFERS_BOOTLOADER; index++ ) 
  {
                              ...
                              // is not expected
    {
      ...
      //For RM_BLANK_CHECK, this indicates the flash page (1 or 2) if blank check fails
