...
CPU_INT08U scriptQueue[MAX_QUEUED_SCRIPTS] = {0};
CPU_INT08U scriptQueueIndex = 0;
...
CPU_INT08U AddScriptToQueue(CPU_INT08U scriptPtr)
{
  ...
  //roundrobinscripts won't be queued if there 
  if (scriptPtr == ROUND_ROBIN_SCRIPT)
  {
    if(scriptQueueIndex >  ROUND_ROBIN_SKIP_QUEUE )
    {
  ...
  if(scriptQueueIndex >= MAX_QUEUED_SCRIPTS) 
  {
    memset(scriptQueue, 0, sizeof(scriptQueue));
    OSSemSet(&ScriptScheduler_Sem, 0, &err);  
    scriptQueueIndex = 0;
    return 3; //error: too many scripts queued
  }
  scriptQueue[scriptQueueIndex] = scriptPtr;
  scriptQueueIndex++;  

  //trigger Script Task to run next script in queue
  OSSemPost(&ScriptScheduler_Sem, OS_OPT_POST_1, &err);  
  if (err !=OS_ERR_NONE)
  {
    return 6;
  }
...
/**
* @brief Shifts the Script Queue down by one.  Call after a script has finished executing, or if the Round Robin 
* has completed and found no scripts to execute.
*/
void ShiftScriptQueue(void)
{
    if(scriptQueueIndex == 0)
      return;
    
    
    //push all scripts down in the script queue, and move index down
    for(CPU_INT08U i = 0; i< MAX_QUEUED_SCRIPTS-1; i++)
      scriptQueue[i] = scriptQueue[i+1];
    
    scriptQueue[ MAX_QUEUED_SCRIPTS - 1] = 0;
    scriptQueueIndex--;
...
CPU_INT08U LoadGlobalVarTable (CPU_INT08U scriptPointer)
{
  ...
  for (i = 0; i < sizeof(globalVarOffset); i++)
    globalVarOffset[i] = 0;
...
{ 
  ...
  while ( TRUE ) 
  {
      ...
      while (childScriptCounter--)
      {
        ...
        if(Control_SystemControl & BIT4) 
        {
          if(ScriptDebug_Indication & BIT0)
            IO0SET = BIT1; //JML DEBUG - See IOInit in app.c for debug usage
          
          scriptErr = RunScriptInterpreter(scriptPointer, &childScriptPointer);
          
          if(ScriptDebug_Indication & BIT0)
            IO0CLR = BIT1; //JML DEBUG - See IOInit in app.c for debug usage
        }
      ...
      if (counter++ > SAVE_TO_FLASH_COUNTER)
      {
        //UpdateFileDirectory(1);
        counter = 0;
      }
      ...
      //script finished executing, so take it off the queue
      ShiftScriptQueue();
